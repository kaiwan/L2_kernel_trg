...
static ssize_t taskinfo_read(struct file *filp, char __user *buf, size_t count, loff_t *offp)
{
	struct task_struct *p;
	char *kbuf, tmp[128];
	int numread = 0, num = 0;

	kbuf = kzalloc(MAXKBUF_LEN, GFP_KERNEL);
	if (!kbuf)
		return -ENOMEM;
...
static const struct file_operations taskinfoopen_fops = {
